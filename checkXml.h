	...
	public:
		void init(char* InpBuf)
		{
			...
			if(strlen(InpBuf) > s -  InpBuf +  checkStrLen)
			{
				...
				{
					int check_close_tag = 8;
					int len = s - InpBuf + check_close_tag;
					inputBuf.resize(len+1);
					CopyMemory(inputBuf.data(), InpBuf, len);
					inputBuf[len] = '\0';

					ofstream inputBuf_without_checkdata("inputBuf_without_checkdata.txt",ios::out|ios::binary);
					inputBuf_without_checkdata << inputBuf.data();
					inputBuf_without_checkdata.close();
...
//					s2 = strstr(InpBuf, "</CHECKDATA>");
		...
		char* getToken(rapidxml::xml_node<> *configuration)
		{
			...
			if(checkdata != NULL)
			{
				checkPlaceCode = getIntAttribute(checkdata, "order_cat");
			}
			else
			{
				  checkPlaceCode = -1;
			...
			{
				auto loyltyApiToken = placeOfService->first_node("LoyltyApiToken");
				if(loyltyApiToken->first_attribute("Default") != NULL &&
					string(loyltyApiToken->first_attribute("Default")->value()) == "true")
				{
					defaultToken =  loyltyApiToken->value();
                }
				int configPlaceCode = atoi(placeOfService->first_node("PlaceCode")->value());
				if(configPlaceCode == checkPlaceCode)
				{
					token = loyltyApiToken->value();
					break;
				}
			}
		...
		{
			if(checkdata == NULL)
				return getJsonResponseForGetDiscountWithEmtyOrder(clientId, res);
